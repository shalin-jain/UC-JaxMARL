...
class GridMapCircleAgents(Map):
    ...
    @partial(jax.jit, static_argnums=[0])
    def biased_sample_test_case(self, key):
        """ Sample a test case for a given map """
        key, _key = jax.random.split(key)
        map_data = self._gen_base_grid()
        ...
        def _body_idx(val, key):
            ...
            i, case, intersect, valid_mask = val
            def _cond_pos(val):
                ...
                # add logic to check for intersecting paths
                def check_path_intersection(other_agent, is_valid):
                    start1, goal1 = pair[:, :2]
                    start2, goal2 = other_agent[:, :2]
                    return jnp.where(is_valid, line_intersection(start1, goal1, start2, goal2), False)

                def line_intersection(start1, goal1, start2, goal2):
                    def ccw(A, B, C):
                        return (C[1]-A[1]) * (B[0]-A[0]) > (B[1]-A[1]) * (C[0]-A[0])

                    ccw1 = ccw(start1, start2, goal2)
                    ccw2 = ccw(goal1, start2, goal2)
                    ccw3 = ccw(start1, goal1, start2)
                    ccw4 = ccw(start1, goal1, goal2)

                    intersect1 = jnp.logical_xor(ccw1, ccw2)
                    intersect2 = jnp.logical_xor(ccw3, ccw4)
            ...
            i += 1
            return (i, case, intersect, valid_mask), None

        fill_max = jnp.max(jnp.array(self.map_size)) + self.rad*2
        case = jnp.full((self.num_agents, 2, 3), fill_max)  # [num_agents, [start_pose, goal_pose]]

        # determine whether to generate intersecting or non intersecting paths
        key_i, key = jax.random.split(key)
        intersect = jax.random.uniform(key_i) < self.intersection_p

        # mask to avoid dynamically sized array in intersection checking logic
        valid_mask = jnp.zeros(self.num_agents, dtype=bool)

        i = 0
        key_scan = jax.random.split(key, self.num_agents)
        (_, case, _, _), _ = jax.lax.scan(_body_idx, (i, case, intersect, valid_mask), key_scan)
